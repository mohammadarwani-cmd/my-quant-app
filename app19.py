...
def main():
    ...
    # 这里的diff包含了 市场盈亏 + 定投金额
    # 很难精确复原每笔定投时刻，这里用近似算法：
    # 策略净值(Net Value) = 总资产 / 投入本金 (不严谨，这是累计倍数)
    # 严谨做法：单位净值法 (Unit NAV)。类似基金，定投是买入份额。
    ...
    # 每天：资产变动率 = 涨跌幅。
    # 资金流入 = 份额增加，不影响 NAV。
    # 所以 NAV 的走势其实就是 "一次性投入" 的走势 (忽略定投买入点位的优劣)
    # 但用户也想看 "账户总盈亏"。
    
    # 我们保留 "Total Return" 为账户总收益率 (Total Assets / Total Invested - 1)
    # 保留 "CAGR" 等指标基于 NAV (即策略本身的表现)

...
TRANSACTION_COST = 0.0001  # 1/10000
...
# ==========================================
# 2. Data Layer - Pure Historical Data
# ==========================================
...
def download_market_data(codes_list, end_date_str):
    """
    Pure historical data download, no real-time fusion
    """
    start_str = '20150101' 
    price_dict = {}
    name_map = {}
...
def main():
    ...
    with tab1:
        ...
        for i, asset in enumerate(overlay_assets):
            s = sliced_data[asset]
            # Normalize to 1.0 at start to compare with strategy
            if not s.empty:
                s_norm = s / s.iloc[0]
                color = colors[i % len(colors)]
                fig.add_trace(go.Scatter(x=s.index, y=s_norm, name=f"{name_map.get(asset, asset)}", line=dict(color=color, width=1, dash='dot'), opacity=0.7), row=1, col=1)

        # Drawdown Area
        dd_series = (df_res['策略净值'] / df_res['策略净值'].cummax() - 1)
        fig.add_trace(go.Scatter(x=df_res.index, y=dd_series, name="回撤", fill='tozeroy', line=dict(color='#e74c3c', width=0), opacity=0.3), row=2, col=1)
    ...
    with tab2:
        ...
        monthly_table = monthly_ret.to_frame(name='Return')
        ...
        # Heatmap
        fig_heat = px.imshow(pivot_table, text_auto='.1%', color_continuous_scale='RdBu_r', midpoint=0)
        fig_heat.update_layout(title="月度回报热力图")
    ...
    with tab3:
        ...
        if not df_log.empty:
            # Filter rows with operations
            df_ops = df_log[df_log['操作'] != ""].copy()
            st.dataframe(df_ops[['日期', '操作', '总资产', '当前持仓']], use_container_width=True)
            
            with st.expander("查看每日详细持仓数据"):
